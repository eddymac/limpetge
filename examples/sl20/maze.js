...
function Maze(dims, seed)
{
    ...
    function advance(curtarget, thisused, otherused)
    {
        for(;;) {   // Loop for start advanced
            ...
            var success = false;
            for(var i = 0; i < 6; i++) {   // See what direction we can go
                tdir += fdir;
                if(tdir < 0) tdir += 6;
                if(tdir >= 6) tdir -= 6;
            ...
            if(success) {         // Increased start tunnel 
                ...
                for(var j = 0; j < 6; j++) {            // For every direction
...
Maze.prototype = {
    ...
    setwall: function(wall, value)
    {
        ...
        pos = ncell;
        var ncell = pos[wall[1]];
        if(!ncell) {
            ncell = [];
            pos[wall[1]] = ncell;
        }
        pos = ncell;
        var ncell = pos[wall[2]];
        if(!pos) {
            ncell = 0x0;
        }
        pos[wall[2]] |= value;
